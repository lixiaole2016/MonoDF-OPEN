...
def build_optimizer(cfg_optimizer, model):
    ...
    new_lr_mult = float(cfg_optimizer.get('new_module_lr_mult', 1.0))
    new_module_keys = tuple(cfg_optimizer.get(
        'new_module_keywords',
        ['geo_', 'gqr_', 'geo_cross_attn', 'geo_norm']))

    for name, param in model.named_parameters():
        is_new_module = any(k in name for k in new_module_keys)
        if is_new_module:
            if 'bias' in name:
                new_biases += [param]
            else:
    ...
    if new_lr_mult != 1.0:
        new_lr = cfg_optimizer['lr'] * new_lr_mult
        if new_biases:
            parameters.append({'params': new_biases, 'weight_decay': 0, 'lr': new_lr})
        if new_weights:
            parameters.append({'params': new_weights, 'weight_decay': cfg_optimizer['weight_decay'], 'lr': new_lr})
    else:
        if new_biases:
            parameters[0]['params'] += new_biases
        if new_weights:
            parameters[1]['params'] += new_weights

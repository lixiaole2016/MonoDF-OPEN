...
from .depth_predictor import DepthPredictor, build_depth_predictor
...
class MonoDF(nn.Module):
    """ MonoDF model: MonoDGP backbone with optional MonoDF (Depth Anything) extensions.

    Stage A: this is identical to MonoDGP. Stage B/C will graft the depth
    foundation model and object-centric geometry memory on top.
    """
    def __init__(self, backbone, depth_predictor, det2d_transformer, det3d_transformer,
        ...
        # Cache whether the chosen depth predictor accepts ``images=...`` so we
        # don't pay the cost of try/except in the hot loop.
        import inspect as _inspect
        try:
            sig = _inspect.signature(depth_predictor.forward)
            self._depth_predictor_takes_images = 'images' in sig.parameters
        except (TypeError, ValueError):
            self._depth_predictor_takes_images = False
        # OGM expects the depth predictor to also output a pure geometry feature.
        self.use_ogm = bool(use_ogm)
        self._depth_predictor_returns_geometry = bool(
            getattr(depth_predictor, 'enable_geometry_output', False))
        if self.use_ogm and not self._depth_predictor_returns_geometry:
            raise ValueError(
                'use_ogm=True requires a depth predictor with '
                'enable_geometry_output=True (set model.enable_geometry_output: True in the config).')
        ...
        if with_box_refine:
            ...
            self.det3d_transformer.decoder.bbox_embed = None

    def set_epoch(self, epoch):
        if hasattr(self.det3d_transformer, 'set_epoch'):
            self.det3d_transformer.set_epoch(epoch)
    ...
    def forward(self, images, calibs, targets, img_sizes, dn_args=None):
        ...
        srcs = enhanced_srcs
        if self._depth_predictor_takes_images:
            depth_out = self.depth_predictor(
                srcs, masks[1], seg_embed[1] + pos[1], images=images)
        else:
            depth_out = self.depth_predictor(
                srcs, masks[1], seg_embed[1] + pos[1])

        if self._depth_predictor_returns_geometry:
            pred_depth_map_logits, depth_pos_embed, weighted_depth, geometry_feat = depth_out
        else:
            pred_depth_map_logits, depth_pos_embed, weighted_depth = depth_out
            geometry_feat = None
        ...
        if self.use_ogm and geometry_feat is not None:
            # Re-use the level-1 (1/16 res) positional encoding and mask;
            # ``geometry_feat`` is produced at the same resolution.
            hs, init_reference, inter_references = self.det3d_transformer(
                intermediate_output, query_embeds, depth_pos_embed,
                geometry_memory=geometry_feat,
                geometry_pos=pos[1],
                geometry_mask=masks[1])
        else:
            hs, init_reference, inter_references = self.det3d_transformer(
                intermediate_output, query_embeds, depth_pos_embed)
...
class SetCriterion(nn.Module):
    """ MonoDF criterion (identical to MonoDGP in Stage A). """
    ...
    def forward(self, outputs, targets, mask_dict=None, epoch=None):
        # epoch arg accepted for backward-compat with current trainer wrappers.
...
def build(cfg):
    ...
    depth_predictor = build_depth_predictor(cfg)
